...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "dashboard":
        ...
        if metric == "revenue":
            ...
            for o in orders:
                if o.get("SelectedEditor") and o.get("Status") not in ["Completed", "Canceled", "Denied"]:
                    ...
                    if not editor_username:
                        warn += " [Editor username missing: Please ask the editor to /start and send their Telegram username. If this changes, orders cannot be delivered properly!]"
                    if not client_username:
                        warn += " [Client username missing: Please ask the client to /start and send their Telegram username. If this changes, orders cannot be delivered properly!]"
            ...
            # One button per applicant with Assign & Skip
            app_buttons = []
            for app in apps:
            ...
            # Show a button per pending order
            order_buttons = []
            for o in pending[:20]:  # limit to 20 to avoid oversized keyboards
                oid = o.get("OrderID", "?")
                cat = o.get("Category", "?")
            ...
            text += f"Tap any order below to review details and Approve / Deny:"
            ...
            if not order:
                await query.edit_message_text(
                    f"⚠️ Order <code>{order_id}</code> not found in Sheets.",
                    parse_mode="HTML",
            ...
            # Use HTML parse mode — safe for URLs, drive links, usernames with underscores, etc.
            view_text = (
                f"📄 <b>ORDER {order_id}</b>\n\n"
                f"👤 Client: <code>{order.get('Client')}</code>\n"
                f"🎬 Category: {order.get('Category', '-')}\n"
                f"⏱ Duration: {order.get('Duration', '-')}\n"
                f"📹 Videos: {order.get('Videos', '-')}\n"
                f"💰 Client Budget: ₹{order.get('ClientBudget', '-')}\n"
                f"✂️ Editor Gets: ₹{order.get('EditorBudget', '-')}\n"
                f"📈 Platform Profit: ₹{order.get('PlatformProfit', '-')}\n"
                f"⏰ Deadline: {order.get('Deadline', '-')}\n"
                f"📁 Raw Files: {order.get('RawFiles', '-')}\n"
                f"🔗 Reference: {order.get('Reference', '-')}\n"
        ...
        # 1. Immediately acknowledge — replace the button message with a processing state
        try:
            await query.edit_message_text(f"⏳ Processing Order <code>{order_id}</code>...", parse_mode="HTML")
        except Exception:
            pass
        
        # 2. Run blocking Sheets calls in a thread so they don't freeze the event loop
        try:
            order = await asyncio.to_thread(get_order, order_id)
        except Exception as e:
            ...
            await context.bot.send_message(
                ...
                text=f"❌ Failed to fetch Order <code>{order_id}</code> from database: {e}",
                parse_mode="HTML"
            )
            return
        ...
        if not order:
            await context.bot.send_message(
                chat_id=update.effective_user.id,
                text=f"❌ Order <code>{order_id}</code> not found in the database.",
                parse_mode="HTML"
            )
            return
        ...
        if order.get("Status") == "Approved":
            await context.bot.send_message(
                chat_id=update.effective_user.id,
                text=f"ℹ️ Order <code>{order_id}</code> is already approved.",
                parse_mode="HTML"
            )
            return
        
        # 3. Update order status in Sheets (in thread)
        try:
            ...
            await context.bot.send_message(
                chat_id=update.effective_user.id,
                text=f"❌ Failed to update order status: {e}",
                parse_mode="HTML"
            )
            return
        
        # 4. Post to Jobs Channel
        posted = False
        post_error = None
        if JOBS_CHANNEL_ID:
            job_text = (
                ...
                f"⏱ Duration: {order.get('Duration', '-')}\n"
                f"📹 Videos: {order.get('Videos', '-')}\n"
                f"💰 Editor Budget: ₹{order.get('EditorBudget', '-')}\n"
                f"⏰ Deadline: {order.get('Deadline', '-')}\n\n"
                f"📁 Raw Files: {order.get('RawFiles', '-')}\n"
                f"🔗 Reference: {order.get('Reference', '-')}"
            )
        ...
        # 5. Notify client (non-blocking, best effort)
        ...
        # Fetch real editor profile from Sheets
        ...
        if msg_id and JOBS_CHANNEL_ID:
            ...
            try:
                # Remove the apply button and change text
                await context.bot.edit_message_text(
                    chat_id=JOBS_CHANNEL_ID,
                    message_id=msg_id,
                    text=f"🔒 <b>{order_id} - Assigned to @{editor_username}</b>",
                    parse_mode="HTML"
                )
            except Exception as e:
        ...
        if action_type == "collected":
            ...
            try:
                ...
                pass
            editor_chat_id = editor.get("TelegramID") if editor else editor_id
            if editor_chat_id:
                await context.bot.send_message(
                    chat_id=editor_chat_id,
                    text=f"✅ Payment secured for Order {order_id}. You may start your work now!"
                )
            if order:
                await context.bot.send_message(
                    chat_id=order.get('Client'),
        ...
        elif action_type == "notyet":
            await query.edit_message_text(f"Payment not yet collected for {order_id}. Editor will be notified once payment is secured.")
            editor = None
            try:
                from database.sheets import get_editor
                editor = get_editor(editor_id)
            ...
            if editor_chat_id:
                await context.bot.send_message(
                    chat_id=editor_chat_id,
                    text=f"⏳ Please wait. Payment for Order {order_id} is not yet secured. Start your work only after confirmation."
                )
        elif action_type == "cancel":
            await query.edit_message_text(f"Order {order_id} has been canceled.")
            update_order_status(order_id, "Canceled")
            editor = None
            try:
                from database.sheets import get_editor
            ...
            if editor_chat_id:
                await context.bot.send_message(
                    chat_id=editor_chat_id,
                    text=f"❌ Sorry, your order {order_id} has been canceled."
                )
            order = get_order(order_id)
            if order:
                await context.bot.send_message(chat_id=order.get('Client'), text=f"Order {order_id} was canceled.")
            # Generate invite link to the Jobs Channel
            invite_link = None
            if JOBS_CHANNEL_ID:
                try:
            ...
            if JOBS_CHANNEL_ID:
                ...
                welcome_text = (
                    ...
                    f"🔹 **Skill:** {skill}\n"
                    f"⚡ **Starting Aura:** 50\n\n"
                    f"📌 *Please check out the Pinned Messages to learn about our rules, the Aura system, and how to use the Jobs workflow!*\n"
            ...
            await query.edit_message_text(f"❌ Rejected test submission for Editor `{editor_id}`.")
            try:
                await context.bot.send_message(
                    chat_id=editor_id,
        ...
        if f"temp_editor_{editor_id}" in context.bot_data:
            ...
            # notify editor and client optionally
